...
base_path = os.path.join(current_path, 'resources3')
movie_names = preprocess.get_movie_names(base_path, [])
scripts = get_script_lists(base_path)

for i, blob in enumerate(scripts):
    print("Top words in", movie_names[i])
    scores = {word: tfidf(word, blob, scripts) for word in blob.words}
